...
public class ClosestCrossTest
{
    ...
    public void NoCrossing_ZeroObjects()
    {
        ...
        var res = cf.ClosestCross(ray, new ITraceable[] { }, out t, out p);
        Assert.IsNull(res);
    }
    ...
    public void Crossing_OneObject()
    {
        ...
        var resPlane = cf.ClosestCross(ray, new ITraceable[] { plane }, out t, out p);
        ...
        var resSphere = cf.ClosestCross(ray, new ITraceable[] {sphere}, out t, out p);
        ...
        var resSphere2 = cf.ClosestCross(ray2, new ITraceable[] { sphere }, out t, out p);
    ...
    public void Crossing_ManyObjects()
    {
        ...
        // default
        var res = cf.ClosestCross(ray, new ITraceable[] { plane, triangle, sphere }, out t, out p);
        Assert.AreSame(res, plane);
        Assert.AreEqual(t, 1, DELTA);
        
        // change order in array
        var res2 = cf.ClosestCross(ray, new ITraceable[] { sphere, plane, triangle }, out t, out p);
        Assert.AreSame(res2, plane);
        Assert.AreEqual(t, 1, DELTA);
        
        // adjust sphere radius to be in front of plane
        sphere.Radius = 6.1;
        var res3 = cf.ClosestCross(ray, new ITraceable[] { sphere, plane, triangle }, out t, out p);
